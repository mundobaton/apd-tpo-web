...
<section id="deposito-pedido">
	<% 
	PedidoDTO pedido = AdministracionDelegate.getInstance().findPedidoById(Long.parseLong(request.getParameter("pid")));
	if(pedido != null){
	...
	<nav aria-label="breadcrumb">
		<ol class="breadcrumb">
			...
			<li class="breadcrumb-item"><a href="<%=request.getContextPath()%>/despacho/index.jsp">Pedidos a Despachar</a></li>
			<li class="breadcrumb-item active" aria-current="page">Ver Pedido #<span class="id"><%=pedido.getId()%></span>
			</li>
		</ol>
	</nav>
	...
	<h2 class="title text-muted">
		...
		<% if(pedido.getEstado() == EstadoPedidoDTO.GENERADO || pedido.getEstado() == EstadoPedidoDTO.PREAPROBADO){%>
			<span class="badge badge-info"><%=pedido.getEstado().getName()%></span>
		<%}else if(pedido.getEstado() == EstadoPedidoDTO.SALDO_INSUFICIENTE || pedido.getEstado() == EstadoPedidoDTO.RECHAZADO){%>
			<span class="badge badge-danger"><%=pedido.getEstado().getName()%></span>
		<%}else if(pedido.getEstado() == EstadoPedidoDTO.PENDIENTE || pedido.getEstado() == EstadoPedidoDTO.A_FACTURAR){%>
			<span class="badge badge-warning"><%=pedido.getEstado().getName()%></span>
		<%}else if(pedido.getEstado() == EstadoPedidoDTO.COMPLETO || pedido.getEstado() == EstadoPedidoDTO.FACTURADO){%>
